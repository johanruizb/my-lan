    ...
    <div className="flex flex-col gap-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Configuración de MyLAN Desktop.</CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="default-profile" className="flex items-center gap-1.5 text-xs text-muted-foreground">
              <Gauge className="h-3.5 w-3.5" aria-hidden />
              Perfil de scan por defecto
            </label>
            <ProfileSelect
              value={settings.default_profile}
              onChange={(v) => setSettingsState({ ...settings, default_profile: v })}
              className="w-40"
              id="default-profile"
            />
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex gap-2" role="group" aria-label="Selector de tema">
              <Button
              ...
              >
                ...
                Claro
                {theme === "light" && <Badge variant="success" className="ml-1">Activo</Badge>}
              </Button>
              <Button
              ...
              >
                ...
                Oscuro
                {theme === "dark" && <Badge variant="success" className="ml-1">Activo</Badge>}
              </Button>
            </div>
            <p className="text-xs text-muted-foreground">
              El tema se aplica inmediatamente y se persiste en los ajustes.
            </p>
          </div>

          <Button onClick={handleSave} disabled={saving} className="w-fit gap-1.5">
            {saving ? (
              <>
                <Loader2 className="h-4 w-4 animate-spin" aria-hidden />
                Guardando…
