...
export function Dashboard() {
  ...
  return (
    <div className="flex flex-col gap-6" aria-busy={scanning}>
      <section aria-label="Red activa">
        <Card>
          ...
          <CardContent className="grid gap-4 sm:grid-cols-2">
            <Info label="Interfaz" value={iface?.name ?? "—"} icon={NetworkIcon} />
            <Info label="IP / CIDR" value={iface ? `${iface.ip}/${iface.prefix_len}` : "—"} icon={Cpu} />
            <Info label="Gateway" value={iface?.gateway_ip ?? "—"} icon={RouterIcon} />
            <Info label="MAC" value={iface?.mac ?? "—"} icon={NetworkIcon} />
            <Info
              label="DNS"
      ...
      <section aria-label="Resumen">
        <div className="grid gap-4 sm:grid-cols-3">
          <Stat label="Dispositivos" value={devices.length} icon={NetworkIcon} />
          <Stat
            label="Hosts vivos (último scan)"
            value={lastScan?.hosts_alive ?? 0}
            icon={Radio}
          />
          <Stat label="Nuevos (último scan)" value={lastScan?.hosts_new ?? 0} icon={Radio} />
        </div>
      </section>
        ...
        <Card>
          ...
          <CardContent className="flex flex-wrap items-center gap-4">
            <div className="flex flex-col gap-1.5">
              <label htmlFor="dash-profile" className="text-xs text-muted-foreground">
                Perfil
              </label>
              <ProfileSelect
                value={profile}
