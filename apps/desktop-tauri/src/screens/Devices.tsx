      ...
      <Card>
        <CardHeader className="flex-row items-center justify-between">
          <CardTitle className="flex items-center gap-2">
            <NetworkIcon className="h-5 w-5 text-primary" aria-hidden />
            Dispositivos ({filtered.length})
          </CardTitle>
          <div className="flex flex-wrap items-center gap-2">
            {/* Toggle de vista tarjetas/tabla (AC-9). */}
            <div className="flex rounded-md border border-border p-0.5" role="group" aria-label="Modo de vista">
              <Button
        ...
        <CardContent>
          ...
          {!error && !loading && filtered.length > 0 && view === "cards" && (
            <div
                  ...
                  >
                    <CardContent className="flex flex-col gap-3 p-4">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div className="flex flex-col">
                            <span className="font-medium">{d.primary_ip ?? d.id}</span>
                            <span className="text-xs text-muted-foreground">
                              {d.hostname ?? d.display_name ?? "—"}
                            </span>
                          </div>
                        </div>
                      ...
                      <dl className="grid grid-cols-2 gap-x-3 gap-y-1 text-xs">
                        <Meta label="MAC" value={d.primary_mac ?? "—"} mono />
                        <Meta label="Vendor" value={d.vendor ?? "—"} />
                        <Meta label="Visto" value={d.last_seen_at} />
                        <Meta label="Confianza" value={d.confidence} />
                      </dl>
                    </CardContent>
                  </Card>
                );
          ...
          {!error && !loading && filtered.length > 0 && view === "table" && (
                ...
                <TableBody>
                  {filtered.map((d) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <Badge variant="secondary">{deviceLabel(d.device_type)}</Badge>
                      </TableCell>
