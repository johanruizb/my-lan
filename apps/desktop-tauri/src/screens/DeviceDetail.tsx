  ...
  if (error)
    return (
      <EmptyState
        ...
        description={error}
        action={
          <Button variant="outline" size="sm" onClick={() => navigate("/devices")} className="gap-1.5">
        ...
        <Card>
          <CollapsibleTrigger asChild>
            <CardHeader>
              <CardTitle className="flex w-full items-center gap-2">
                ...
                <Badge variant="secondary" className="ml-1">{deviceLabel(d.device_type)}</Badge>
                <ChevronDown
                  className={cn("ml-auto h-4 w-4 transition-transform", openInfo ? "" : "-rotate-90")}
                  aria-hidden
                />
              </CardTitle>
            </CardHeader>
          ...
          <CollapsibleContent>
            <CardContent className="grid gap-3 sm:grid-cols-2">
              ...
              <Field label="Tipo" value={deviceLabel(d.device_type)} />
          ...
          <CollapsibleTrigger asChild>
            <CardHeader>
              <CardTitle className="flex w-full items-center gap-2">
                ...
                Escaneo de puertos
                <ChevronDown
                  className={cn("ml-auto h-4 w-4 transition-transform", openScan ? "" : "-rotate-90")}
                  aria-hidden
                />
              </CardTitle>
            </CardHeader>
            ...
            <CardContent className="flex flex-col gap-4">
              <div className="flex flex-wrap items-center gap-3">
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="detail-profile" className="text-xs text-muted-foreground">
                    Perfil
                  </label>
                  <ProfileSelect
                    value={profile}
                ...
                <Button onClick={handleScanPorts} disabled={scanning} className="mt-5 gap-1.5">
                  {scanning ? (
                    <>
                      <Loader2 className="h-4 w-4 animate-spin" aria-hidden />
                      Escaneando…
          ...
          <CollapsibleTrigger asChild>
            <CardHeader>
              <CardTitle className="flex w-full items-center gap-2">
                ...
                <ChevronDown
                  className={cn("ml-auto h-4 w-4 transition-transform", openServices ? "" : "-rotate-90")}
                  aria-hidden
                />
              </CardTitle>
            </CardHeader>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {detail.services.map((s) => {
                        const StateIcon = serviceStateIcon(s.state);
                        return (
                          <TableRow key={s.id}>
                            <TableCell className="uppercase">{s.protocol}</TableCell>
                            ...
                            <TableCell>
                              <Badge
                                variant={s.state.toLowerCase() === "open" ? "success" : "outline"}
                                className="gap-1"
                              >
                                <StateIcon className="h-3 w-3" aria-hidden />
                                {serviceStateLabel(s.state)}
                              </Badge>
                            </TableCell>
